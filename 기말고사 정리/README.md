...
## 목차
  - [7주차: 고급 위젯과 이벤트 처리](#7주차--고급-위젯과-이벤트-처리)
    - [1](#1-)
    - [2)](#2-)
    - [3](#3-)
    - [4](#4-)
...
- **4/17(목)** : 8. 고급 위젯과 이벤트 처리 (2) / 9. 액티비티와 명시적 인텐트  

## ✅ 8주차
- **4/22(화)** : 중간고사 기간 1차시 휴강  
- **4/24(목)** : **중간고사**  
...
### 1.1. 이벤트 드리븐(Event‑Driven)  
- 사용자 입력(클릭, 터치 등)이 발생했을 때 그에 반응하는 방식  
- **리스너(listener)** 가 등록된 위젯에서 콜백 메서드를 호출
...
```
```java
CheckBox cb = findViewById(R.id.checkA);
cb.setOnCheckedChangeListener((button, isChecked) -> {
  // isChecked = true/false
});
```

### 2.2. RadioButton & RadioGroup (단일 선택)
```xml
<RadioGroup android:id="@+id/radioGroup">
  <RadioButton android:id="@+id/rb1" android:text="옵션 1"/>
  <RadioButton android:id="@+id/rb2" android:text="옵션 2"/>
</RadioGroup>
```
```java
...
rg.setOnCheckedChangeListener((group, checkedId) -> {
  switch (checkedId) {
    case R.id.rb1: /* 처리 */ break;
    case R.id.rb2: /* 처리 */ break;
  }
});
```
...
```
```java
ToggleButton tb = findViewById(R.id.toggleBtn);
tb.setOnCheckedChangeListener((button, isOn) -> {
...
1. **이벤트 처리**  
   - XML vs. 리스너 vs. 람다, 장단점 구분  
2. **컴파운드 버튼**  
   - CheckBox(다중), RadioButton(단일), ToggleButton/Switch(토글)  
3. **RatingBar**  
   - 별점 수(numStars), 증가단계(stepSize) 설정  
4. **실습 포인트**  
   - 각 위젯의 `setOn…Listener` 메서드 파라미터와 콜백 시그니처 숙지  
